...
@Component({
    selector: 'jhi-story-my-suffix',
    templateUrl: './story-my-suffix.component.html'
})
export class StoryMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.storyService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<StoryMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/story-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/story-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: StoryMySuffix) {
        return item.id;
    }
